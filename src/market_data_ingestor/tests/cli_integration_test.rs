...
fn test_single_fetch_cli() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(feather_count, 1, "Expected 1 feather file, found {}", feather_count);
...
fn test_batch_fetch_cli_with_file_input() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let feather_count = count_feather_files(stdout);
    assert_eq!(feather_count, 2, "Expected 2 feather files, found {}", feather_count);
...
fn test_batch_fetch_cli_with_stdin_input() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::new("/home/hanbo/repo/stock_trading_bot/src/target/debug/market_data_ingestor")
        .args([
            "--config",
            config_path,
            "batch",
...
fn test_batch_fetch_cli_with_invalid_source() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // This should fail
    assert!(!output.status.success(), "Binary should have failed with invalid source");
...
fn test_batch_fetch_cli_missing_required_input() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // This should fail
    assert!(!output.status.success(), "Binary should have failed with missing input");
