...
class DBBenchmark:
    ...
    def benchmark_duckdb(self):
        ...
        conn.close()
        return {'db': 'DuckDB', 'create_time': create_time, 'insert_time': insert_time, 'query_time': query_time}
    ...
    def benchmark_sqlite(self):
        ...
        with timer() as query_timer:
            cursor = conn.cursor()
            for symbol in self.stock_symbols[:10]:
                cursor.execute("SELECT * FROM stock_data WHERE timestamp >= '2020-06-01'")
                cursor.fetchall()
        query_time = query_timer()

        conn.close()
        return {'db': 'SQLite', 'create_time': create_time, 'insert_time': insert_time, 'query_time': query_time}
    ...
    def visualize_results(self):
        ...
        print(f"Benchmark visualization saved to {results_dir}/duckdb_vs_sqlite_benchmark.html")
