...
pub fn read_current(conn: &mut SqliteConnection) -> anyhow::Result<Current> {
    use crate::schema::{provider, asset_class, provider_asset_class, provider_symbol_map};
    ...
    let pairs = provider_asset_class::table
        .select((provider_asset_class::provider_code, provider_asset_class::asset_class_code))
        .load::<(String,String)>(conn)?
        .into_iter().collect();
    ...
    Ok(Current { providers, classes, pairs, symbols })
}