...
schema = Schema(
    fields=[
        ...
        NestedField(
            field_id=2, name="event_time", field_type=TimestamptzType(), required=True
        ),  # Using TimestamptzType for timezone
...
partition_spec = PartitionSpec(
    ...
    PartitionField(
        field_id=3,
        source_id=schema.find_field("level").field_id,
        name="level_identity",
...
# Load your catalog with the properties n
...
catalog.create_table(
    full_table_name, schema, location=None, partition_spec=partition_spec
)
