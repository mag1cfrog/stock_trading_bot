...
class StorageDesignBenchmark:
    def __init__(
        self,
        symbol_counts: list = SYMBOL_COUNTS,
        row_counts: list = ROW_COUNTS,
        append_batch_number: int = APPEND_BATCH_NUMBER,
    ...
    def generate_synthetic_data(self):
        ...
        initial_timestamps = pl.datetime_range(
    ...
    def benchmark_option_1(self):
        ...
        with timer() as create_timer:
            for symbol in self.stock_symbols:
                for timeframe in self.timeframes:
                    ...
                    conn.execute(
                        f"CREATE TABLE {symbol}_{timeframe} AS SELECT * FROM temp_data"
                    )
                ...
                with timer() as batch_timer:
                    for symbol in self.stock_symbols:
                        for timeframe in self.timeframes:
                            ...
                            conn.execute(
                                f"INSERT INTO {symbol}_{timeframe} SELECT * FROM append_data"
                            )
        ...
        with timer() as query_timer:
            for symbol in self.stock_symbols:
                for timeframe in self.timeframes:
                    conn.execute(
                        f"SELECT * FROM {symbol}_{timeframe} WHERE timestamp >= '2020-06-01'"
                    )
    ...
    def benchmark_option_3(self):
        ...
        with timer() as create_timer:
            for symbol in self.stock_symbols:
                ...
                for timeframe in self.timeframes:
                    conn.execute(
                        f"CREATE TABLE data_{timeframe} AS SELECT * FROM temp_data"
                    )
                conn.close()
        create_time = create_timer()
            ...
            for batch_idx, batch_data in enumerate(self.append_batches):
                with timer() as batch_timer:
                    for symbol in self.stock_symbols:
                        ...
                        for timeframe in self.timeframes:
                            conn.execute(
                                f"INSERT INTO data_{timeframe} SELECT * FROM append_data"
                            )
                        conn.close()
                append_times.append(batch_timer())
        total_append_time = total_append_timer()
        ...
        with timer() as query_timer:
            for symbol in self.stock_symbols:
                ...
                for timeframe in self.timeframes:
                    conn.execute(
                        f"SELECT * FROM data_{timeframe} WHERE timestamp >= '2020-06-01'"
                    )
                conn.close()
        query_time = query_timer()
        ...
        for num_symbols, num_rows in scenarios:
            ...
            for repeat in range(self.num_repeats):
                ...
                for option in range(1, 4):
                    ...
                    scenario_results[f"option_{option}"]["create_times"].append(
                        option_results["create_time"]
                    )
                    scenario_results[f"option_{option}"]["append_times"].append(
                        option_results["append_time"]
                    )
                    scenario_results[f"option_{option}"]["query_times"].append(
                        option_results["query_time"]
                    )
                    total_time = (
                        option_results["create_time"]
                        + option_results["append_time"]
                    ...
                    scenario_results[f"option_{option}"]["total_times"].append(
                        total_time
                    )

                    # Store append times per batch
                    scenario_results[f"option_{option}"][
                        "append_times_per_batch"
                    ].append(option_results["append_time_per_batch"])
            ...
            for option in range(1, 4):
                for metric in [
                    ...
                    std_time = (
                        sum((x - avg_time) ** 2 for x in times) / len(times)
                    ) ** 0.5
                    scenario_results[f"option_{option}"][f"avg_{metric[:-1]}"] = (
                        avg_time
                    )
                    scenario_results[f"option_{option}"][f"std_{metric[:-1]}"] = (
                        std_time
                    )
    ...
    def analyze_results(self, results=None):
        ...
        for scenario, scenario_results in results.items():
            ...
            for option in range(1, 4):
                ...
                print(f"\nOption {option}:")
                print(
                    f"  Create Time: {opt_results['avg_create_time']:.2f}s ± {opt_results['std_create_time']:.2f}s"
                )
                print(
                    f"  Append Time: {opt_results['avg_append_time']:.2f}s ± {opt_results['std_append_time']:.2f}s"
                )
                print(
                    f"  Query Time: {opt_results['avg_query_time']:.2f}s ± {opt_results['std_query_time']:.2f}s"
                )
                print(
                    f"  Total Time: {opt_results['avg_total_time']:.2f}s ± {opt_results['std_total_time']:.2f}s"
                )
    ...
    def visualize_results(self, results):
        ...
        for metric, title in metrics.items():
            fig = make_subplots(
                ...
                cols=2,
                subplot_titles=[
                    f"{n} Symbols" for n in sorted(df["num_symbols"].unique())
                ],
            ...
            print(
                f"Benchmark visualization for {title} saved to {results_dir}/benchmark_results_{metric}.html"
            )
        ...
        print(
            f"Append batch performance visualization saved to {results_dir}/benchmark_results_append_batches.html"
        )
